 ...
 *
 * ■ いまの状態
 *   2026-09-01 に値を入れて有効化した。アカウント 223-961-7825 に作った
 *   「無料相談の申し込み（フォーム送信）」と「電話番号のタップ（サイト内）」の2つを測る。
 *   カウント方法はどちらも「初回のみ」。自動タグ設定はオンであることを確認済み。
 *   CONFIG.id を空にすれば、外部スクリプトを一切読み込まない状態に戻せる。
 *
 * ■ 値の取り直し方（Google広告の管理画面 / アカウント 223-961-7825）
 *   1. ツールと設定 → 測定 → コンバージョン
...
(function () {
  ...
  var CONFIG = {
    id: 'AW-18385662568',            // アカウント 223-961-7825 の Google タグ
    formLabel: 'zv7iCPDfs-scEOjk-75E', // 無料相談の申し込み（フォーム送信）
    telLabel: 'NJ23CPPfs-scEOjk-75E'   // 電話番号のタップ（サイト内）
  };
