...
- 変更前後で `node scripts/check-performance-budget.mjs` を実行する。PRでは `.github/workflows/performance.yml` がモバイル／PCのLighthouseを計測し、モバイル88点・LCP3.8秒、PC95点・LCP2秒を下回る明確な退行を止める。
