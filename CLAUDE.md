...
- **サービス一覧は横長・行型リスト（6行・01〜06）**（一度「主要3サービスの写真カード化 .svc-top3」を入れたがユーザー判断で撤回・削除済み）。トップは `home-svc-*`、`services.html` は `svc-ill-*` の4:3イラスト用 `<image-slot>` を使う。表示幅はPC 112px・モバイル72px。行は `<a>` なのでラッパー span に編集環境限定のクリックガードを付け、公開時のリンク操作とClaude Designの編集を両立している。正本画像は `assets/illustrations/service-*.webp`。画像内の実色 `#fefefe` との境界が見えないよう、サービス6行とトップ記事一覧3行はいずれも、行・画像枠・`image-slot` の背景を同色に統一する。成長段階・記事一覧・事務所紹介・`joseikin.html` も4:3画像＋`fit="contain"`で統一し、画像内の人物・小物を切らない。事務所紹介の4枚は画像内に意味があるため、重複する説明ラベルを重ねない。
...
本番前要確認（2026-07-23更新）：電話・代表名・LINE撤去・OGP画像・ドメイン・連絡先メール（`contact@minano-sr.com`）は反映済み。登録番号は未確定情報を掲載しない状態。残り＝番号確定後の正式情報追加／FormSubmitの新アドレスでの再有効化・受信テスト／GoatCounterアカウント登録。

## 2026-07 トップ短縮・4ページ分割

- トップは `01 理念`／`02 サービス`／`03 支援の進め方`／`04 料金`／`05 FAQ`／`06 お知らせ・コラム`／`07 事務所案内` の短い入口に再構成。詳細情報の正本は以下の4ページへ分離した。
  - `services.html`：支援姿勢・対応ツール・6サービス・助成金支援
  - `pricing.html`：顧問プラン・スポット料金・助成金報酬・料金シミュレーター
  - `support.html`：成長段階・業種別支援例・相談から契約までの流れ
...
## 監査ツール（数値を記憶で語らないための道具・2026-07-30 追加）
...
- `node scripts/audit-a11y.cjs http://127.0.0.1:8811/`（playwright必要）… 全公開ページ×9画面幅で、ページ自体の横スクロール・背景色から算出したコントラスト・操作領域24px・console/pageエラー。**写真の上の文字は背景色から計算できない**ので、`PHOTO_TARGETS` に登録した要素だけ「文字を透明にして背景の実ピクセルを測る」方式で別に測る。写真上に文字を置く箇所を増やしたらここに追加する。
