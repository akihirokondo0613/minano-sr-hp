...
品目カードの日付（入社日・退職日）は1名のときだけ聞く。2名以上は各自の日付が違うので欄を出さず、`発生日` を空で送る。各自の日付は Excel の受領フォームで受ける。対象の方の氏名も注文では聞かず（`対象者` は空で送る）、1名は案件ページの記入シート、2名以上は Excel で集める。「ひとこと（任意）」の例文は加入する保険の種類（例：雇用保険のみ加入）。Apps Script 側は、Excel が届いて「送信を完了する」が押されたとき、法定期限が空の案件があれば担当者メールで知らせる（台帳の法定期限は人が入れる）。
